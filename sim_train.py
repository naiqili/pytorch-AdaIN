...
network = net.SimNet(vgg, decoder)
network.train()
network.to(device)
...
for i in tqdm(range(args.max_iter)):
    ...
    loss_c, loss_s = network(content_images)
    ...
    writer.add_scalar('sim_loss_content', loss_c.item(), i + 1)
    writer.add_scalar('sim_loss_style', loss_s.item(), i + 1)
    ...
    if (i + 1) % args.save_model_interval == 0 or (i + 1) == args.max_iter:
        ...
        torch.save(state_dict,
                   '{:s}/sim_decoder_iter_sw0001_{:d}.pth.tar'.format(args.save_dir,
                                                           i + 1))
writer.close()
